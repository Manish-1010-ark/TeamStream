...
function CopyButton({ textToCopy }) {
  ...
  return (
    <button
      onClick={handleCopy}
      // Use conditional classes based on the `isCopied` state to change style
      className={`relative w-24 h-8 rounded-lg font-semibold text-sm transition-all duration-300 overflow-hidden ${
        isCopied
    ...
    >
      ...
      <span
        className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${
          isCopied ? "opacity-100 transform-none" : "opacity-0 -translate-y-full"
        }`}
      >
        Copied!
      </span>

      {/* "Copy ID" span with icon */}
      <span
        className={`absolute inset-0 flex items-center justify-center gap-1.5 transition-all duration-300 ${
          isCopied ? "opacity-0 translate-y-full" : "opacity-100 transform-none"
        }`}
      >
        <svg
          width="12"
